...
public class HarrisCornerDetector: OperationGroup {
    ...
    @objc open var sensitivity:Float = 5.0 { didSet { harrisCornerDetector.uniformSettings["sensitivity"] = sensitivity } }
    @objc open var threshold:Float = 0.2 { didSet { nonMaximumSuppression.uniformSettings["threshold"] = threshold } }
    public var cornersDetectedCallback:(([Position]) -> ())?
    
    @objc open var cornersDetectedOCCallback:((NSArray) -> ())? {
        didSet {
            if cornersDetectedOCCallback != nil {
                cornersDetectedCallback = { ( positions ) in
                    let points:NSMutableArray = NSMutableArray()
                    for position in positions {
                        points.add(NSValue(cgPoint: CGPoint(x: CGFloat(position.x), y: CGFloat(position.y))))
                    }
                    self.cornersDetectedOCCallback!(points)
                }
            }else {
                cornersDetectedCallback = nil
            }
        }
    }
