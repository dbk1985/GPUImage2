...
public class Crop: BasicOperation {
    ...
    @objc open var cropOCSizeInPixels: CGSize = CGSize.zero {
        didSet {
            if cropOCSizeInPixels.equalTo(CGSize.zero) {
                cropSizeInPixels = nil
            }else {
                cropSizeInPixels = Size(width: Float(cropOCSizeInPixels.width), height: Float(cropOCSizeInPixels.height))
            }
        }
    }
    
    @objc open var locationOfCropInPixelsForOC: CGPoint = CGPoint.zero {
        didSet {
            locationOfCropInPixels = Position(point: locationOfCropInPixelsForOC)
        }
    }
